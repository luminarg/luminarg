      ...
      <div className="mx-auto max-w-6xl space-y-10">
        <header>
          ...
          <p className="mt-3 text-neutral-400">
            Gestión interna de pedidos de clientes, pagos, entregas e impacto de stock.
          </p>
        </header>
                  ...
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0 space-y-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span
                            className={`text-sm ${statusClass(sale.status)}`}
                          >
                      ...
                      <div className="grid gap-2 text-sm text-neutral-400 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <p>
                          Pago:{" "}
                          <span className={statusClass(sale.payment_status)}>
                        ...
                        <p>
                          Entrega:{" "}
                          <span className={statusClass(sale.delivery_status)}>
                      ...
                      <div className="text-sm text-neutral-500">
                        Entrega: {sale.shipping_street}{" "}
                        {sale.shipping_street_number},{" "}
                    ...
                    <div className="flex shrink-0 flex-col gap-2 sm:flex-row lg:flex-col">
                      <Link
                        ...
                        className="rounded-full border border-white/10 px-4 py-2 text-center text-sm text-neutral-300 hover:border-white/30"
                      >
                        Ver detalle
                      </Link>
                      ...
                      {sale.payment_status !== "paid" &&
                        sale.status !== "cancelled" && (
                          <form action={markSaleAsPaidAction.bind(null, sale.id)}>
                            <button
                              type="submit"
                              className="w-full rounded-full bg-white px-4 py-2 text-sm font-medium text-black"
                            >
                              Marcar pagada
                            </button>
                          </form>
                      ...
                      {sale.payment_status === "paid" &&
                        ...
                        sale.delivery_status !== "delivered" && (
                          ...
                          >
                            <button
                              type="submit"
                              className="w-full rounded-full border border-white/10 px-4 py-2 text-sm text-neutral-300 hover:border-white/30"
                            >
                              Avanzar entrega
                            </button>
                          </form>
                      ...
                      {sale.status !== "cancelled" && (
                        <form action={cancelSaleAction.bind(null, sale.id)}>
                          <button
                            type="submit"
                            className="w-full rounded-full border border-red-500/30 px-4 py-2 text-sm text-red-300 hover:border-red-500/60"
                          >
                            Cancelar
                          </button>
                        </form>
